...
import type { GeneratedAgentRecord } from '@/lib/utils/database';
...
export type PersistedGeneratedAgent = Pick<
  GeneratedAgentRecord,
  'id' | 'name' | 'role' | 'persona' | 'avatar' | 'color' | 'priority' | 'voiceConfig'
>;
...
function clearGeneratedAgentsFromRegistry() {
...
export function hydrateGeneratedAgentsForStage(
  stageId: string,
  agents: PersistedGeneratedAgent[],
): string[] {
  clearGeneratedAgentsFromRegistry();
