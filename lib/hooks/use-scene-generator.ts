...
async function generateTTSForScene(
  ...
  return {
    // TTS is optional for testing and should not block scene generation.
    success: true,
    failedCount,
    error: lastError,
  };
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
              ...
              if (ttsResult.failedCount > 0) {
                log.warn(
                  `TTS failed for ${ttsResult.failedCount} speech actions in scene "${scene.title}", continuing without audio.`,
                );
              }
            }
    ...
    async (outlineId: string) => {
      ...
      try {
        ...
        if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
          ...
          if (ttsResult.failedCount > 0) {
            log.warn(
              `Retry finished with ${ttsResult.failedCount} TTS failures for scene "${actionsResult.scene.title}", continuing without audio.`,
            );
          }
        }
