      ...
      <div className={cn('flex flex-col w-full h-full overflow-hidden', collapsed && 'hidden')}>
        ...
        <div className="h-10 flex items-center justify-between shrink-0 relative mt-3 mb-1 px-3">
          <button
            ...
            title={brandTitle || t('generation.backToHome')}
          >
            {brandImageSrc ? (
              <img src={brandImageSrc} alt={brandAlt} className="h-6" />
            ) : (
              <span className="max-w-[150px] truncate text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-300">
                {brandLabel || brandAlt}
              </span>
            )}
          </button>
